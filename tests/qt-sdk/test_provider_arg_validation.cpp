...
// {"code":"dispatch_failed", ...}. These tests pin the two Qt provider sites to
// that same rule:
//
//   * the GENERATED dispatch (logos-cpp-generator --provider-header), which
//     emits logos::qtArgFromVariant<T> per parameter — SampleProvider below is
//     generated by the real generator at build time;
...
// ═══════════════════════════════════════════════════════════════════════════
// 2. The GENERATED dispatch — logos-cpp-generator --provider-header.
//    SampleProvider's callMethod is produced by the real generator at build
//    time (see tests/qt-sdk/CMakeLists.txt), so this exercises the emitted
//    code, not a hand-written stand-in.
// ═══════════════════════════════════════════════════════════════════════════

class GeneratedDispatchArgTest : public ::testing::Test {
protected:
    void SetUp() override { m_provider = new SampleProvider(); }
    void TearDown() override { delete m_provider; }
    SampleProvider* m_provider = nullptr;
};

TEST_F(GeneratedDispatchArgTest, FractionalFloatForIntArgIsRejected)
{
    // add(int, int) used to run with (4, 1).
    const QVariant r = m_provider->callMethod("add", {QVariant(3.7), QVariant(1.2)});
    EXPECT_TRUE(isDispatchFailed(r)) << r.toString().toStdString();
    EXPECT_EQ(m_provider->lastA, 0) << "the method body must not have run";
    EXPECT_EQ(r.toMap().value(QStringLiteral("origin")).toString(),
              QStringLiteral("sample_provider"));
}

TEST_F(GeneratedDispatchArgTest, NonStringElementInStringListArgIsRejected)
{
    // resolveDependencies(QStringList) used to run with ["a", "1"].
    const QVariant r = m_provider->callMethod(
        "resolveDependencies", {QVariant(QVariantList{QVariant("a"), QVariant(1)})});
    EXPECT_TRUE(isDispatchFailed(r)) << r.toString().toStdString();
    EXPECT_TRUE(m_provider->lastDependencies.isEmpty());
    EXPECT_NE(rejectionMessage(r).indexOf(QStringLiteral("arg0[1]")), -1)
        << rejectionMessage(r).toStdString();
}

TEST_F(GeneratedDispatchArgTest, NumberForBoolArgIsRejected)
{
    const QVariant r = m_provider->callMethod(
        "installPlugin", {QVariant("/path"), QVariant(1)});
    EXPECT_TRUE(isDispatchFailed(r)) << r.toString().toStdString();
    EXPECT_TRUE(m_provider->lastPluginPath.isEmpty());
}

TEST_F(GeneratedDispatchArgTest, NumberForStringArgIsRejected)
{
    const QVariant r = m_provider->callMethod("echo", {QVariant(5)});
    EXPECT_TRUE(isDispatchFailed(r)) << r.toString().toStdString();
    EXPECT_TRUE(m_provider->lastEcho.isEmpty());
}

TEST_F(GeneratedDispatchArgTest, WholeValuedFloatForIntArgStillDispatches)
{
    const QVariant r = m_provider->callMethod("add", {QVariant(3.0), QVariant(4.0)});
    EXPECT_FALSE(isDispatchFailed(r));
    EXPECT_EQ(r.toInt(), 7);
    EXPECT_EQ(m_provider->lastA, 3);
    EXPECT_EQ(m_provider->lastB, 4);
}

TEST_F(GeneratedDispatchArgTest, WellFormedArgsStillDispatch)
{
    EXPECT_EQ(m_provider->callMethod("echo", {QVariant("hello")}).toString(),
              QStringLiteral("hello"));
    EXPECT_TRUE(m_provider->callMethod("installPlugin",
                                       {QVariant("/p"), QVariant(true)}).toBool());
    EXPECT_EQ(m_provider->callMethod("add", {QVariant(2), QVariant(3)}).toInt(), 5);

    m_provider->callMethod("resolveDependencies",
                           {QVariant(QStringList{"a", "b"})});
    EXPECT_EQ(m_provider->lastDependencies, (QStringList{"a", "b"}));

    // void return: still the true sentinel, not a rejection.
    EXPECT_TRUE(m_provider->callMethod("setDirectory", {QVariant("/tmp")}).toBool());
    EXPECT_EQ(m_provider->lastDirectory, QStringLiteral("/tmp"));
}

// ═══════════════════════════════════════════════════════════════════════════
// 3. The QMetaObject dispatch — QtProviderObject::callMethod. This is the
